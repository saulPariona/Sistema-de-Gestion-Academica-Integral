...
@section('contenido')
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-3xl font-bold text-primary mb-1">Banco de Preguntas</h1>
    ...
    <div class="grid gap-4">
                ...
                <div class="flex justify-between items-start">
                    <div class="flex-1">
                        <div class="flex gap-2 mb-3">
                            <span class="px-3 py-1 rounded-full text-xs font-bold {{ $pregunta->dificultad == 'facil' ? 'bg-green-100 text-green-700 border border-green-300' : ($pregunta->dificultad == 'medio' ? 'bg-yellow-100 text-yellow-700 border border-yellow-300' : 'bg-red-100 text-red-700 border border-red-300') }}">
                                {{ ucfirst($pregunta->dificultad) }}
                            </span>
                            <span class="px-3 py-1 rounded-full text-xs font-bold bg-primary/10 text-primary border border-primary/30">
                        ...
                        @if ($pregunta->imagen)
                            <img src="{{ asset('storage/' . $pregunta->imagen) }}" alt="Imagen"
                                class="mt-2 max-h-40 rounded-lg border-2 border-gray-200 shadow-md">
                        @endif
                        <div class="mt-3 space-y-1.5">
                            @foreach ($pregunta->alternativas as $alt)
                                <div class="flex items-center gap-2 text-sm p-2 rounded-lg {{ $alt->es_correcta ? 'bg-green-50 border-2 border-green-300 text-green-800 font-semibold' : 'bg-gray-50 border border-gray-200 text-gray-700' }}">
                                    @if($alt->es_correcta)
                    ...
                    <div class="flex gap-2 ml-4">
                        ...
                        <form method="post" action="{{ route('docente.preguntas.eliminar', [$curso, $pregunta]) }}"
                            ...
                            @csrf
                            @method('DELETE')
                            <button class="flex items-center gap-1 bg-red-600 text-white px-3 py-2 rounded-lg text-xs font-semibold hover:bg-red-700 transition-all shadow-md">
                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @empty
            <div class="bg-white rounded-xl shadow-lg p-12 text-center border-2 border-dashed border-gray-300">
                ...
                <p class="text-gray-500 font-medium text-lg">No hay preguntas registradas</p>
            </div>
        @endforelse
    </div>
