...
@section('contenido')
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-3xl font-bold text-primary mb-1">Editar Examen</h1>
        ...
        <form method="post" action="{{ route('docente.examenes.actualizar', [$curso, $examen]) }}">
            ...
            <div class="mb-5">
                ...
                <textarea name="descripcion" rows="3"
                    class="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all">{{ old('descripcion', $examen->descripcion) }}</textarea>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">Puntaje Total</label>
                    <input type="number" name="puntaje_total"
                        value="{{ old('puntaje_total', $examen->puntaje_total) }}" min="1"
                        class="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all" required>
                    @error('puntaje_total')
                ...
                <div>
                    ...
                    <input type="number" name="tiempo_limite"
                        value="{{ old('tiempo_limite', $examen->tiempo_limite) }}" min="1"
