...
@section('contenido')
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-3xl font-bold text-primary mb-1">Gestión de Usuarios</h1>
            <p class="text-gray-600">Administrar estudiantes, docentes y personal</p>
        </div>
    ...
    <form method="get" class="bg-white rounded-xl shadow-lg p-4 mb-6 border-2 border-primary/20">
        <div class="flex flex-wrap gap-3">
            <input type="text" name="buscar" value="{{ request('buscar') }}" placeholder="Buscar por nombre, DNI o email..."
                class="flex-1 min-w-[250px] p-2 text-sm border-2 border-gray-300 rounded-lg focus:border-primary focus:outline-none">
            <select name="rol" class="p-2 text-sm border-2 border-gray-300 rounded-lg focus:border-primary focus:outline-none">
            ...
            <select name="estado" class="p-2 text-sm border-2 border-gray-300 rounded-lg focus:border-primary focus:outline-none">
            ...
            <div class="bg-white rounded-xl shadow-lg p-6 border-2 border-primary/20 hover:shadow-2xl transition-all">
                <div class="flex justify-between items-start">
                    <div class="flex items-start gap-4 flex-1">
                        <div class="bg-{{ $usuario->rol == 'administrador' ? 'purple' : ($usuario->rol == 'docente' ? 'green' : 'blue') }}-100 p-4 rounded-lg">
                            <svg class="w-8 h-8 text-{{ $usuario->rol == 'administrador' ? 'purple' : ($usuario->rol == 'docente' ? 'green' : 'blue') }}-600" fill="currentColor" viewBox="0 0 20 20">
                        ...
                        <div class="flex-1">
                            <h3 class="text-xl font-bold text-gray-800 mb-2">{{ $usuario->nombreCompleto() }}</h3>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-2 text-sm text-gray-600">
                                ...
                                <div>
                                    <span class="font-semibold">Último acceso:</span> {{ $usuario->ultimo_acceso?->format('d/m/Y H:i') ?? '-' }}
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="flex flex-wrap gap-2 ml-4">
                        ...
                        <form method="post" action="{{ route('admin.usuarios.toggle', $usuario->id) }}" class="inline">
                            ...
                            @method('PATCH')
                            <button type="submit"
                                class="flex items-center gap-1 {{ $usuario->estado == 'activo' ? 'bg-red-600 hover:bg-red-700' : 'bg-green-600 hover:bg-green-700' }} text-white px-3 py-2 rounded-lg text-xs font-semibold transition-all shadow-md">
                        ...
                        <form method="post" action="{{ route('admin.usuarios.reset-password', $usuario->id) }}" class="inline">
                            @csrf
                            @method('PATCH')
                            <button type="submit"
                                class="flex items-center gap-1 bg-orange-600 text-white px-3 py-2 rounded-lg text-xs font-semibold hover:bg-orange-700 transition-all shadow-md"
                        ...
                        @if ($usuario->rol == 'estudiante')
                            <a href="{{ route('admin.apoderados', $usuario->id) }}"
                                class="flex items-center gap-1 bg-indigo-600 text-white px-3 py-2 rounded-lg text-xs font-semibold hover:bg-indigo-700 transition-all shadow-md">
                                <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
