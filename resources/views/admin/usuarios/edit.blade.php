...
@section('contenido')
    ...
    <form method="post" action="{{ route('admin.usuarios.actualizar', $user->id) }}" enctype="multipart/form-data"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">Nueva Contraseña (dejar vacío para no
                    cambiar)</label>
            ...
            <div>
                ...
                <select name="rol" class="w-full p-2 text-sm border border-gray-300 rounded">
                    <option value="estudiante" {{ old('rol', $user->rol) == 'estudiante' ? 'selected' : '' }}>Estudiante
                    </option>
            ...
            <div>
                ...
                <select name="estado" class="w-full p-2 text-sm border border-gray-300 rounded">
                    ...
                    <option value="inactivo" {{ old('estado', $user->estado) == 'inactivo' ? 'selected' : '' }}>Inactivo
                    </option>
                    <option value="bloqueado" {{ old('estado', $user->estado) == 'bloqueado' ? 'selected' : '' }}>
                        Bloqueado</option>
                </select>
            </div>
            ...
            <div>
                ...
                <input type="text" name="especialidad" value="{{ old('especialidad', $user->especialidad) }}"
                    class="w-full p-2 text-sm border border-gray-300 rounded">
            </div>

            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">Grado Académico</label>
                <input type="text" name="grado_academico" value="{{ old('grado_academico', $user->grado_academico) }}"
                    class="w-full p-2 text-sm border border-gray-300 rounded">
            </div>
