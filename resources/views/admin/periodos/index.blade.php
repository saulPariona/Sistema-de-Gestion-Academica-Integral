...
@section('contenido')
    ...
    <div class="grid gap-4">
        @forelse ($periodos as $periodo)
            <div class="bg-white rounded-xl shadow-lg p-6 border-2 {{ $periodo->estado == 'activo' ? 'border-green-300' : 'border-primary/20' }} hover:shadow-2xl transition-all">
                <div class="flex justify-between items-start">
                    <div class="flex-1">
                        <div class="flex items-center gap-3 mb-3">
                            <h3 class="text-xl font-bold text-gray-800">{{ $periodo->nombre }}</h3>
                            <span class="px-3 py-1 rounded-full text-xs font-bold border {{ $periodo->estado == 'activo' ? 'bg-green-100 text-green-700 border-green-300' : 'bg-gray-100 text-gray-700 border-gray-300' }}">
                        ...
                        <div class="flex flex-wrap gap-4 text-sm text-gray-600">
                            <div class="flex items-center gap-2">
                                ...
                                <span><span class="font-semibold">Inicio:</span> {{ $periodo->fecha_inicio->format('d/m/Y') }}</span>
                            </div>
                            <div class="flex items-center gap-2">
                                <svg class="w-4 h-4 text-red-600" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clip-rule="evenodd"/>
                                </svg>
                                <span><span class="font-semibold">Fin:</span> {{ $periodo->fecha_fin->format('d/m/Y') }}</span>
                            </div>
                        </div>
                    </div>
